function SSD = calcSSD(f1,f2, feat1, feat2, n, T, overlapX, overlapY)
    %
    % 'f1'          : First image 
    %
    % 'f2'          : Second image
...
%         end
        ...
        % Else prüfe ob Koordinate bereits gematcht
            ...
            [tf1, rowExist1] = ismember(bestMatch(1,2:3), SSD(:,2:3), 'rows');
            [tf2, rowExist2] = ismember(bestMatch(1,4:5), SSD(:,4:5), 'rows');
            if tf1 == 1
                % If SSD von bestMatch < alte SSD, füge hinzu
                if bestMatch(1,1) < SSD(rowExist1,1)
                    SSD(rowExist1,:) = bestMatch;
                % else verwerfe Match
                else
                    continue;
                end
            ...
            elseif tf2 == 1
                % If SSD von bestMatch < alte SSD, füge hinzu
                if bestMatch(1,1) < SSD(rowExist2,1)
                    SSD(rowExist2,:) = bestMatch;
                % else verwerfe Match
                else
                    continue;
                end 
